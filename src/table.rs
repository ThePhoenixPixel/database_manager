...
pub trait Table: Sized {
    ...
    async fn update<M: DatabaseController>(&self, manager: &M, filters: &QueryFilters) -> DbResult<usize> {
        manager.update(Self::table_name(), filters, &self.to_row()).await
    }

    /// Delete this struct from the database
    async fn delete<M: DatabaseController>(&self, manager: &M, filters: &QueryFilters) -> DbResult<usize> {
        manager.delete(Self::table_name(), filters).await
    }
